...
export default function UserTable() {
  ...
  const handleDelete = async (user: User) => {
    if (confirm(`Atenção, o usuário ${user.username} será excluído permanentemente.`)) {
      try {
      ...
      <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row gap-4 items-center justify-between bg-gray-50/50 dark:bg-gray-800/50">
        <div className="relative w-full sm:w-72">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-sm"
          />
        </div>
        <button 
              ...
              <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                ...
                <td className="p-4">
                  <div className="flex gap-1 flex-wrap">
                    {(user.roles ?? []).map((r) => (
                      <span key={r.name} className="px-2.5 py-1 rounded-md text-xs font-medium bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">
